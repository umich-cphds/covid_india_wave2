...
source("figures/figure s8/esir_ally.R")
...
# load data ----------
obs <- fread("figures/figure s8/covid19india_national_counts_20211031.csv")[
  , `:=` (
    date     = as.Date(date),
    scenario = "Observed"
  )][
...
get_pred <- function(x, r_0 = 2) {
  ...
  tmp_4wk_pred <- fread(glue("figures/figure s8/{tmp_4wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "4 weeks", start_date = as.Date(x, "%Y%m%d"))][]
  tmp_6wk_pred <- fread(glue("figures/figure s8/{tmp_6wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "6 weeks", start_date = as.Date(x, "%Y%m%d"))][]
  tmp_8wk_pred <- fread(glue("figures/figure s8/{tmp_8wk_casename}_data.txt"))[, date := as.Date(date)][date >= as.Date(x, "%Y%m%d")][, `:=` (scenario = "8 weeks", start_date = as.Date(x, "%Y%m%d"))][]
...
# save output ----------
ggsave(filename =  glue("figures/figure s8/figure s8.pdf"),
       plot     = full_plot,
       height   = 10,
       width    = 10,
