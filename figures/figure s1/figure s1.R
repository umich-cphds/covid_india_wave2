...
fig.init <- data.cum %>% 
  ...
  xlab("") + 
  geom_vline(xintercept = c(as.Date("2021-02-19"), as.Date("2020-06-19"))) + 
  ...
  labs(title = "Plot of new daily cases for waves 1 and 2", 
       subtitle = "Wave 1: June 19, 2020 - February 18, 2021.\nWave 2: February 19, 2021 - July 31, 2021.", 
...
obj1 <- data.cum %>% 
  filter(Date %in% as.Date(c("2020-06-18"))) %>% 
  dplyr::select(-c(Recovered, Deceased, Other, Tested)) %>% 
  filter(State %in% states$st_nm) 

obj2 <- data.cum %>% 
  filter(Date %in% as.Date(c("2021-02-18"))) %>% 
  dplyr::select(-c(Recovered, Deceased, Other, Tested)) %>% 
  filter(State %in% states$st_nm)  
...
cases %>% select(c(State, metric, tier)) %>% 
  full_join(deaths %>% select(c(State, metric, tier)), by = "State") %>% 
  rename(metric.case = metric.x, tier.case = tier.x, 
         metric.death = metric.y, tier.death = tier.y) %>% 
  write_csv(paste0("metrics.csv"))
...
""
subtitle <- glue("Based on data from June 19, 2020 - July 31, 2021.\nWave 1: June 19, 2020 - February 18, 2021.\nWave 2: February 19, 2021 - July 31, 2021.")
...
ggsave("figs1A.png", p.cases, dpi = 300, width = 35, height = 35, units = "cm")
...
p.deaths <- ggplot(states_population %>% 
  ...
  theme(legend.text = element_text(size=12), 
        legend.title = element_text(size=14))
ggsave("figs1B.png", p.deaths, dpi = 300, width = 35, height = 35, units = "cm")
