...
option_list <- list(
  ...
  make_option(c("--edate"),action="store", default="2021-05-15",help="End date for the model [default \"%default\"]"),
  make_option(c("--vdate"),action="store", default="2020-12-01",help="Start of variant date for the model [default \"%default\"]"),
  make_option(c("--jname"),action="store", default="base_maharashtra_simulation",help="Name of stanfile [default \"%default\"]"),
  make_option(c("--wpar"),action="store", type="integer", default=310,help="Wanning Rayleigh Par [default \"%default\"]"),
  make_option(c("--ifr1"),action="store", type="double", default=0.25,help="IFR1 prior [default \"%default\"]"),
  make_option(c("--ur"),action="store", type="double", default=0.5,help="Under Reporting [default \"%default\"]"),
  make_option(c("--cpr"),action="store", type="double", default=1,help="Cross Protection [default \"%default\"]"),
  make_option(c("--areaname"),action="store", default="Maharashtra",help="Which area to run this for [default \"%default\"]"),
  make_option(c("--nchains"),action="store", type="integer", default=2,help="Number of Chains [default \"%default\"]"),
  make_option(c("--warmup"),action="store", type="integer", default=300,help="Number of warmup iterations [default \"%default\"]"),
  make_option(c("--iter"),action="store", type="integer", default=600,help="Number of iterations [default \"%default\"]"),
  make_option(c("--thin"),action="store", type="integer", default=1,help="Amount of thinning of results [default \"%default\"]"),
  make_option(c("--seed"),action="store", type="integer", default=12111,help="Seed for run [default \"%default\"]"),
...
df_pop <- import(here("section_2_two_strain_model/", "data", "population.csv"))
serial.interval <- import(here("section_2_two_strain_model/", "data","serial_interval.csv"))
onset_paras <-
  import(here("section_2_two_strain_model/", "data","statelevel_OnsetDeathParams.csv")) %>%
  select(region,mean,cv)
pcr_and_sero <-import(here("section_2_two_strain_model/", "data","pcr_and_sero_pos.csv"))
if(opt$areaname ==  "Maharashtra")
{
  pcr_file = "pcr_genome_fraction_maharashtra.csv"
} else {
  print("wrong location")
}

pcr_genome_fraction <-
  import(here("section_2_two_strain_model/", "data",pcr_file)) %>%
...
if(length(stan_data$N) == 1) {
  ...
  stan_data$ifr1_prior = as.array(stan_data$ifr1_prior)
}
...
m = stan_model(here(paste0('section_2_two_strain_model/stan_model/',StanModel,'.stan')))



## if running on rstuid use this to have terminal free and make sure to comment
## fit call right now not commented
# job::job(
#   {fit = sampling(m,
#                   data=stan_data,
#                   iter=ITER,
...
if(!dir.exists(here(paste0("section_2_two_strain_model/figures/",job)))){
  dir.create(here(paste0("section_2_two_strain_model/figures/",job)), recursive = TRUE)
}
if(!dir.exists(here(paste0("section_2_two_strain_model/results/",job)))){
  dir.create(here(paste0("section_2_two_strain_model/results/",job)), recursive = TRUE)
}
...
filename <- paste(opt$jname,gsub("-","_",opt$edate),gsub("-","_",opt$vdate),opt$wpar,opt$areaname,opt$iter,opt$ur, opt$ifr1,opt$cpr,sep="_")
saveRDS(saved_data, here(paste0("section_2_two_strain_model/results/",job,"/",filename,"_saved_data.rds")))


shortsummary=rbind(head(summarise_draws(fit),10), tail(summarise_draws(fit),10))
shortsummary
write.csv(as.data.frame(shortsummary), here(paste0("section_2_two_strain_model/results/",job,"/",filename,"_summary_data.csv")))
