...
sched <- fread("https://raw.githubusercontent.com/maxsal/science_revision/main/pi_schedule_extended.txt?token=AJI5MRDHU2GXFFB56Z76PBDBQKRLA",
               showProgress = FALSE)
tier3 <- sched[place== "Maharashtra early"]
use_these_pis <- tier3[, .(pis = smooth_pis)] # vector of pis
...
saveRDS(Result,"Result_Tier3.rds")

tier3=readRDS("Result_Tier3.rds")
Prediction3=tier3$prediction
dime=(nrow(Prediction3)/14)
P_daily=rowMeans(Prediction3[dime*9+1:dime,])
