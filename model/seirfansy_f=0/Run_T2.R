...
start_date <- as.Date("2021-02-19")
date_end   <- as.Date(start_date - 1)
date_start  <- as.Date(date_end - 99)
...
sched <- fread("https://raw.githubusercontent.com/maxsal/science_revision/main/pi_schedule_extended.txt?token=AJI5MRDHU2GXFFB56Z76PBDBQKRLA",
               showProgress = FALSE)
tier2 <- sched[place== "MH Pre-lock +20%"]
use_these_pis <- tier2[, .(pis = smooth_pis)] # vector of pis
...
Result = model_predictR(data = data_multinomial,init_pars=NULL,data_init = data_initial, T_predict = tpred, pi=pi_f,
                        ...
                        mu = 1/(69.416 * 365), period_start = phases, opt_num = 300, auto.initialize=T,alpha_u=0.5,f=0,
                        plot = T, save_plots = FALSE)


saveRDS(Result,"Result_Tier2.rds")
tier2=readRDS("Result_Tier2.rds")
Prediction2=tier2$prediction
P_daily=rowMeans(Prediction2[dime*9+1:dime,])
dates=seq.Date(from = as.Date(date_start),
               to   = as.Date(date_end+150),
               by    = "day")
require(tidyverse)
pred=as_tibble(P_daily) %>% 
  add_column(dates)

write.csv(pred,"Tier2.csv")
