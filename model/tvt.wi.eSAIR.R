tvt.eSAIR <- function(Y, R,
                      a0 = NULL, 
                      pi0 = NULL, change_time= NULL, exponential = FALSE,
                      omega0 = NULL, change_time_omega = NULL, lambda0 = NULL, rayleigh = FALSE, sigma0 = NULL,
                      multiplier = 1, 
                      ...
                      dic = FALSE,
                      death_in_R = 0.02,
                      gamma0 = 0.0821, gamma0_sd = 0.1,
                      gamma_s0 = 1 / 240, gamma_s0_sd = 0.1,
                      R0 = 2,  R0_sd = 1,
                      use_sero = FALSE,
                      seroprev = 0,
                      kappaprec = 0.0001,
  ...
  ### pi schedule
  ## (multiplied by s(t) externally - will not allow in here)
  ...
  ### a(t) schedule
  if (!is.null(a0)) {
    message("Running for survey-based a(t)")
    alpha_vec <- a0
    alpha_vec[2:length(alpha_vec)] <- 0
  }else {
    message("Running without a(t)")
  ...
  model2.string <- paste0("
                          ...
                          Km[t-1,2] <- -beta*(theta[t-1,1]+0.5*Km[t-1,1])*(theta[t-1,2]+0.5*Km[t-1,5])*pi[t-1] +omega[t-1]*gamma_s*(theta[t-1,3]+0.5*Km[t-1,9])
                          Km[t-1,10] <- gamma*(theta[t-1,2]+0.5*Km[t-1,5])-omega[t-1]*gamma_s*(theta[t-1,3]+0.5*Km[t-1,9]) 
                          Km[t-1,6] <- -Km[t-1,2]-Km[t-1,10]

                          Km[t-1,3] <- -beta*(theta[t-1,1]+0.5*Km[t-1,2])*(theta[t-1,2]+0.5*Km[t-1,6])*pi[t-1]+omega[t-1]*gamma_s*(theta[t-1,3]+0.5*Km[t-1,10])
                          Km[t-1,11] <- gamma*(theta[t-1,2]+0.5*Km[t-1,6])-omega[t-1]*gamma_s*(theta[t-1,3]+0.5*Km[t-1,10])
                          Km[t-1,7] <- -Km[t-1,3]-Km[t-1,11]

                          Km[t-1,4] <- -beta*(theta[t-1,1]+Km[t-1,3])*(theta[t-1,2]+Km[t-1,7])+omega[t-1]*gamma_s*(theta[t-1,3]+Km[t-1,11]) 
                          ...
                          alpha_temp[t-1,3] <- max(theta[t-1,3]+(Km[t-1,9]+2*Km[t-1,10]+2*Km[t-1,11]+Km[t-1,12])/6, 0)
                          ...
                          theta_A[1] <- 0
                          theta_H[1] <- 0
                          theta0[1:3]<-c(", 1 - Y[1]/(1-sero) - R[1]/(1-sero), ",", Y[1]/(1-sero), ",", R[1]/(1-sero), ")

                          theta[1,1:3] ~ ddirch(theta0[1:3])

                          gamma ~  dlnorm(", lognorm_gamma_parm$mu,",", 1 / lognorm_gamma_parm$var,")
                          
                          gamma_s ~  dlnorm(",lognorm_gamma_s_parm$mu, ",", 1/lognorm_gamma_s_parm$var,")
                          
                          R0 ~ dlnorm(",lognorm_R0_parm$mu, ",",  1 / lognorm_R0_parm$var, ")

                          beta <- R0*gamma

                          k ~  dgamma(",2, ",", kappaprec, ")

                          lambdaY ~ dgamma(", 2, ",", lambdaYprec, ")

                          lambdaR ~ dgamma(", 2, ",", lambdaRprec, ")
  }")
  ...
  if (save_files) {
    ...
    ## include output for theta_H (tbdl)
    ...
    lambdaR_p <- unlist(as.mcmc.list(jags_sample$lambdaR))
    k_p <- unlist(as.mcmc.list(jags_sample$k))
    # thetaunlist(as.mcmc.list(jags_sample$theta_A))
  ...
  for (l in 1:len) {
    ...
    if (betal < 0 | gammal < 0 | gamma_sl < 0 | thetalt1 < 0 | thetalt2 < 0 | thetalt3 < 0 | thetaltH < 0 | thetaltA < 0) next
    for (t in 1:(T_fin - T_prime)) {
      Km <- NULL
      theta_temp <- alpha_temp <- alpha <- NULL
      
      Km[1] <- -betal*pi[t+T_prime]*thetalt1*thetalt2 + omega[t+T_prime]*gamma_sl*thetalt3
      Km[9] <- gammal*thetalt2 - omega[t+T_prime]*gamma_sl*thetalt3
      Km[5] <- -Km[1] -Km[9]
      
      Km[2] <- -betal*(thetalt1+0.5*Km[1])*(thetalt2+0.5*Km[5])*pi[t+T_prime]+omega[t+T_prime]*gamma_sl*(thetalt3+0.5*Km[9])
      Km[10] <- gammal*(thetalt2+0.5*Km[5])-omega[t+T_prime]*gamma_sl*(thetalt3+0.5*Km[9])
      Km[6] <- -Km[2]-Km[10]
      
      Km[3] <- -betal*(thetalt1+0.5*Km[2])*(thetalt2+0.5*Km[6])*pi[t+T_prime]+omega[t+T_prime]*gamma_sl*(thetalt3+0.5*Km[10])
      Km[11] <- gammal*(thetalt2+0.5*Km[6])-omega[t+T_prime]*gamma_sl*(thetalt3+0.5*Km[10])
      Km[7] <- -Km[3]-Km[11]
      
      Km[4] <- -betal*(thetalt1+Km[3])*(thetalt2+Km[7])+omega[t+T_prime]*gamma_sl*(thetalt3+Km[11])
      Km[12] <- gammal*(thetalt2+Km[7])-omega[t+T_prime]*gamma_sl*(thetalt3+Km[11])
      Km[8] <- -Km[4]-Km[12]
