model_plotR <- function(Result = NULL, mcmc_pars = NULL, T_predict = NULL, T_simu = NULL, n_period, data,
  ...
  if(call == "predictR"){
    ...
    RR_current_pred_mean = prediction_mean[5 * T_simu + 1:T_simu]
    DR_current_pred_mean = prediction_mean[7 * T_simu + 1:T_simu]
    ...
    RR_current_pred_2.5 = prediction_2.5[5 * T_simu + 1:T_simu]
    DR_current_pred_2.5 = prediction_2.5[7 * T_simu + 1:T_simu]
    ...
    RR_current_pred_97.5 = prediction_97.5[5 * T_simu + 1:T_simu]
    DR_current_pred_97.5 = prediction_97.5[7 * T_simu + 1:T_simu]
    ...
    data_detected_undetected_plot = data.frame("date" = rep(1:T_simu, times = 2),
                                               "count" = c( U_current_pred, P_current_pred),
                                               "type" = factor(rep(c("Untested", "Confirmed"), each = T_simu), levels = c("Untested", "Confirmed")))
    ...
    plot_detected_undetected <- ggplot(data_detected_undetected_plot, aes(x = date, y = count)) +
      ...
      labs(title="Current Cases - Untested and Detected", x = "days from onset", y = "cases") +
      scale_y_continuous(labels = scales::comma)
