par_initializeR <- function(data,model="Multinomial", init_pars,end_beta, period_start, init_state_num, fix_pars, opt_num = 100,... ) {
  ...
  negLL_func<- function(pars,period_start = parent.frame()$period_start, init_state_num = parent.frame()$init_state_num, 
    ...
    p_new_pred_n <- round(ypred[, 10])
    p_new_pred_prob <- ypred[, 11]
    rd_new_pred_n <- round(ypred[, 12])
    r_new_pred_prob <- ypred[, 13]
    d_new_pred_prob <- ypred[, 14]
    p <- c()
    if(model=="Poisson"){
  ...
  while(i <= opt_num) {
    ...
    while (!passed) {
      if(is.null(init_pars)){
        initial_param<-c(runif(n_period,0.1,end_beta),rep(0.2,times=n_period))
        end_beta
        }
     else{
