# daily deaths w/ intervals and cumulative death w/ interval
pacman::p_load(tidyverse, here, janitor, glue)
source(here("lockdown", "extract_cfr.R"))
source(here("lockdown", "get_esir_incidence.R"))
...
quick_death_sum <- function(base_path, scenarios, cfr_scenario, end_date, i) {
  ...
  # p_end <- p %>% filter(date == end_date) %>% pull(death_cumulative) %>% round()
  
  # step 3: rupam's cumlative death interval -----------
  # death_int    <- get_esir_death(scen = scenarios[i], alpha = "india", end_date = end_date)
  # death_factor <- p_end / death_int$mid
  # death_int    <- death_int %>% mutate(across(where(is.numeric), \(x) round(x * death_factor)))
  ...
  # step 4: rupam's death incidence interval -----------
  ...
  # peak_inc <- inc_dat %>% pull(death_incidence) %>% max(., na.rm = T)
  
  # inc_dat %>% filter(death_incidence == max(death_incidence, na.rm = T)) %>% mutate(across(where(is.numeric), \(x) round(x)))
  
  # inc_dat %>%
  #   filter(date <= end_date) %>%
  #   ggplot(aes(x = date, y = death_incidence)) +
  #   geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  #   geom_line(size = 1) +
  #   coord_cartesian(ylim = c(0, peak_inc * 100)) +
...
obs <- d %>%
  ...
  death_int_out %>%
    ...
    as.data.frame() %>%
    write_tsv(glue("~/Downloads/{file_name}.txt"))

  save(lst, file = glue("~/Downloads/{file_name}.Rdata"))

# for (i in seq_along(scenarios)) {
#   if (i == 1) {
#     p <- lst[[i]][["death_dat"]]
#   } else {
#     p <- bind_rows(p, lst[[i]][["death_dat"]])
#   }
# }
# p <- p %>% mutate(value = round(est))
# 
# proj <- p %>%
...
#   select(date, lower, value, upper, scenario)
# 
# 
# lst$`2021-03-01`
# 
