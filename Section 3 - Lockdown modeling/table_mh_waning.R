# libraries ------------
librarian::shelf(
  tidyverse, here, glue, janitor, anytime, zoo
)
...
for (i in seq_along(scenarios)) {
  ...
  if (i == 1) {
    p <- read_tsv(here("Section 3 - Lockdown modeling", "data", "waning",
                       tmp_filename),
                  col_types = cols()) %>%
      mutate(scenario = scenarios[i])
  } else {
    p <- bind_rows(p,
                   read_tsv(here("Section 3 - Lockdown modeling", "data", "waning",
                                 tmp_filename),
                            col_types = cols()) %>%
                     mutate(scenario = scenarios[i]))
  }
...
# save tables -----------
if (mh == TRUE) {
  case_out  <- here("Section 3 - Lockdown modeling", "out", "table_mh_mhs_cases.txt")
  death_out <- here("Section 3 - Lockdown modeling", "out", "table_mh_mhs_deaths.txt")
} else if (kl == TRUE) {
  case_out  <- here("Section 3 - Lockdown modeling", "out", "table_kl_mhs_cases.txt")
  death_out <- here("Section 3 - Lockdown modeling", "out", "table_kl_mhs_deaths.txt")
} else {
  case_out  <- here("Section 3 - Lockdown modeling", "out", "table_mhs_cases.txt")
  death_out <- here("Section 3 - Lockdown modeling", "out", "table_mhs_deaths.txt")
}
